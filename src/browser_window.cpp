...
bool is_url_candidate(const std::string& text) {
    if (text.empty() || has_whitespace(text)) return false;
...
bool is_history_uri(const char* uri) {
    ...
    if (value.rfind("extart://", 0) == 0) return false;
    return true;
}
...
BrowserWindow::BrowserWindow(
    ...
    g_signal_connect(bookmarks_button_, "clicked", G_CALLBACK(on_bookmarks_button_clicked_cb), this);
    g_signal_connect(history_button_, "clicked", G_CALLBACK(on_history_button_clicked_cb), this);
    g_signal_connect(downloads_button_, "clicked", G_CALLBACK(on_downloads_button_clicked_cb), this);
...
GtkWidget* BrowserWindow::widget() const { return window_; }
...
Tab* BrowserWindow::active_tab() const { return active_tab_; }
...
void BrowserWindow::close_tab(Tab* tab) {
    ...
    if (was_active) {
        const std::size_t next_index = index >= tabs_.size() ? tabs_.size() - 1 : index;
...
void BrowserWindow::on_bookmarks_button_clicked_cb(GtkButton*, gpointer user_data) {
    static_cast<BrowserWindow*>(user_data)->on_bookmarks_button_clicked();
}

void BrowserWindow::on_history_button_clicked_cb(GtkButton*, gpointer user_data) {
    static_cast<BrowserWindow*>(user_data)->on_history_button_clicked();
}

void BrowserWindow::on_downloads_button_clicked_cb(GtkButton*, gpointer user_data) {
...
gboolean BrowserWindow::on_key_pressed(
    ...
    auto* window = static_cast<BrowserWindow*>(user_data);
    const gboolean is_ctrl = state & GDK_CONTROL_MASK;
    const gboolean is_shift = state & GDK_SHIFT_MASK;
    ...
    if (is_ctrl && keyval == GDK_KEY_w) {
        if (window->active_tab_) window->close_tab(window->active_tab_);
        return TRUE;
    }
    if (is_ctrl && keyval == GDK_KEY_Tab) {
        if (window->tabs_.empty()) return TRUE;
        auto it = std::find_if(window->tabs_.begin(), window->tabs_.end(),
            [window](const std::unique_ptr<Tab>& tab) { return tab.get() == window->active_tab_; });
        if (it != window->tabs_.end()) {
            auto next = std::next(it);
            if (next == window->tabs_.end()) next = window->tabs_.begin();
    ...
    if (is_ctrl && is_shift && keyval == GDK_KEY_ISO_Left_Tab) {
        ...
        auto it = std::find_if(window->tabs_.begin(), window->tabs_.end(),
            [window](const std::unique_ptr<Tab>& tab) { return tab.get() == window->active_tab_; });
        if (it != window->tabs_.end()) {
            auto prev = it == window->tabs_.begin() ? std::prev(window->tabs_.end()) : std::prev(it);
...
void BrowserWindow::setup_ui_panels() {
    history_panel_ = std::make_unique<HistoryPanel>(history_.get());
    bookmarks_panel_ = std::make_unique<BookmarksPanel>(bookmarks_.get());
    downloads_panel_ = std::make_unique<DownloadsPanel>(download_manager_.get());

    history_panel_->set_on_entry_activated([this](const std::string& url) {
        Tab* tab = active_tab();
        if (tab == nullptr) tab = &open_tab();
        tab->load_uri(url.c_str());
        if (history_popover_) gtk_popover_popdown(GTK_POPOVER(history_popover_));
    });

    bookmarks_panel_->set_on_entry_activated([this](const std::string& url) {
        Tab* tab = active_tab();
        if (tab == nullptr) tab = &open_tab();
        tab->load_uri(url.c_str());
        if (bookmarks_popover_) gtk_popover_popdown(GTK_POPOVER(bookmarks_popover_));
    });

    bookmarks_panel_->set_on_bookmark_added([this]() {
        if (bookmarks_panel_) bookmarks_panel_->refresh();
    });
}
...
void BrowserWindow::on_history_button_clicked() {
    if (!history_popover_) {
        ...
        GtkWidget* panel = history_panel_->create_panel();
        gtk_popover_set_child(GTK_POPOVER(history_popover_), panel);
        gtk_widget_set_size_request(panel, 420, 500);
        gtk_widget_set_parent(history_popover_, history_button_);
    }

    history_panel_->refresh();
    if (gtk_widget_get_visible(history_popover_)) {
        gtk_popover_popdown(GTK_POPOVER(history_popover_));
    } else {
...
void BrowserWindow::on_bookmarks_button_clicked() {
    if (!bookmarks_popover_) {
        ...
        GtkWidget* panel = bookmarks_panel_->create_panel();
        gtk_popover_set_child(GTK_POPOVER(bookmarks_popover_), panel);
        gtk_widget_set_size_request(panel, 420, 500);
        gtk_widget_set_parent(bookmarks_popover_, bookmarks_button_);
    }
    ...
    if (gtk_widget_get_visible(bookmarks_popover_)) {
        gtk_popover_popdown(GTK_POPOVER(bookmarks_popover_));
    } else {
...
void BrowserWindow::on_downloads_button_clicked() {
    if (!downloads_popover_) {
        ...
        GtkWidget* panel = downloads_panel_->create_panel();
        gtk_popover_set_child(GTK_POPOVER(downloads_popover_), panel);
        gtk_widget_set_size_request(panel, 500, 500);
        gtk_widget_set_parent(downloads_popover_, downloads_button_);
    }

    downloads_panel_->refresh();
    if (gtk_widget_get_visible(downloads_popover_)) {
        gtk_popover_popdown(GTK_POPOVER(downloads_popover_));
    } else {
