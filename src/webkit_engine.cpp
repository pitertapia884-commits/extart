...
WebKitEngine::WebKitEngine(Profile& profile, const Config& config) {
    ...
    web_view_ = GTK_WIDGET(g_object_new(WEBKIT_TYPE_WEB_VIEW, "web-context", profile.web_context(), "network-session", profile.network_session(), "user-content-manager", user_content_manager, nullptr));
...
bool WebKitEngine::can_go_back() const { return web_view_ != nullptr && webkit_web_view_can_go_back(WEBKIT_WEB_VIEW(web_view_)); }
bool WebKitEngine::can_go_forward() const { return web_view_ != nullptr && webkit_web_view_can_go_forward(WEBKIT_WEB_VIEW(web_view_)); }
void WebKitEngine::go_back() { if (can_go_back()) webkit_web_view_go_back(WEBKIT_WEB_VIEW(web_view_)); }
void WebKitEngine::go_forward() { if (can_go_forward()) webkit_web_view_go_forward(WEBKIT_WEB_VIEW(web_view_)); }
void WebKitEngine::reload() { if (web_view_ != nullptr) webkit_web_view_reload(WEBKIT_WEB_VIEW(web_view_)); }
...
void WebKitEngine::stop_loading() { if (web_view_ != nullptr) webkit_web_view_stop_loading(WEBKIT_WEB_VIEW(web_view_)); }

void WebKitEngine::find_text(const std::string& text) {
    if (find_controller_ == nullptr) return;
    last_search_ = text;
    if (text.empty()) { webkit_find_controller_search_finish(find_controller_); return; }
    webkit_find_controller_search(find_controller_, text.c_str(), WEBKIT_FIND_OPTIONS_CASE_INSENSITIVE, G_MAXUINT);
}
void WebKitEngine::find_next() { if (find_controller_ != nullptr && !last_search_.empty()) webkit_find_controller_search_next(find_controller_); }
void WebKitEngine::find_previous() { if (find_controller_ != nullptr && !last_search_.empty()) webkit_find_controller_search_previous(find_controller_); }
void WebKitEngine::clear_find() { if (find_controller_ == nullptr) return; webkit_find_controller_search_finish(find_controller_); last_search_.clear(); }
...
void WebKitEngine::apply_config(const Config& config) {
    ...
    webkit_settings_set_user_agent(settings, "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36");
}

void WebKitEngine::setup_downloads(DownloadManager& manager) {
    if (web_view_ != nullptr) manager.setup_for_web_view(WEBKIT_WEB_VIEW(web_view_));
}
...
void WebKitEngine::on_load_changed(WebKitWebView* view, int event, gpointer user_data) {
    ...
    if (event == WEBKIT_LOAD_COMMITTED && engine->callbacks_.uri_changed) engine->callbacks_.uri_changed(webkit_web_view_get_uri(view));
    if (event == WEBKIT_LOAD_FINISHED) {
        if (engine->callbacks_.title_changed) engine->callbacks_.title_changed(webkit_web_view_get_title(view));
        if (engine->callbacks_.load_finished) engine->callbacks_.load_finished();
    }
}
