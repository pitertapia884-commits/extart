...
WebKitUserContentManager* create_whatsapp_compatibility_manager() {
    ...
    // Keep this compatibility layer isolated to the temporary WebKit backend.
    // The eventual Gecko backend will not inherit WebKit-specific shims.
...
WebKitEngine::WebKitEngine(Profile& profile, const Config& config) {
    ...
    apply_config(config);
    find_controller_ = webkit_web_view_get_find_controller(
        WEBKIT_WEB_VIEW(web_view_)
    );
...
WebKitEngine::~WebKitEngine() {
    // GTK owns the widget hierarchy. BrowserWindow removes the widget before
    // destroying the Tab, so avoid WebKit calls during final object teardown.
    find_controller_ = nullptr;
    web_view_ = nullptr;
}

GtkWidget* WebKitEngine::widget() const {
    return web_view_;
}
...
void WebKitEngine::find_next() {
    if (find_controller_ == nullptr || last_search_.empty()) return;
    ...
    if (web_view_ != nullptr) {
        ...
        if (settings != nullptr) {
            webkit_settings_set_enable_javascript(
                settings, config.javascript_enabled());
            webkit_settings_set_auto_load_images(
                settings, config.images_enabled());
            webkit_settings_set_enable_media(
                settings, config.sound_enabled());
            webkit_settings_set_enable_webaudio(
                settings, config.sound_enabled());
            webkit_settings_set_javascript_can_open_windows_automatically(
                settings, config.popups_enabled());
...
void WebKitEngine::setup_downloads(DownloadManager& manager) {
    if (web_view_ != nullptr) {
        manager.setup_for_web_view(WEBKIT_WEB_VIEW(web_view_));
    }
}

void WebKitEngine::on_load_changed(WebKitWebView* view,
                                    int event,
                                    gpointer user_data) {
